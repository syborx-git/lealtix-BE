...
-- ==============================================================================
-- Garantiza:
--   0. Plantillas Oficiales de Campañas (11 templates)
--   1. Usuarios Propietarios (app_user: admin@demo.com y admin@restaurante-petra.com)
--   2. Tenant Principal (Restaurante Petra / Demo)
--   3. Usuarios Operativos (tenant_user: ADMIN, MESERO, COCINA, CAJA, MARKETING, HOSTESS)
--      con contraseñas Base64 para login en /api/tenant/auth/login
--   4. Configuración del Tenant (tenant_config)
--   5. Mesas del Local (mesa 1 a 4 y Barra con rotación y formas)
--   6. Menú Gastronómico Completo Petra:
--      * 14 Categorías (tenant_menu_category)
--      * 113 Insumos de cocina (insumo)
--      * 29 Sub-recetas (tenant_menu_product con es_sub_receta = true)
--      * 58 Platillos y extras (tenant_menu_product)
--      * Asignación de multicategorías (tenant_menu_product_category)
--      * 332 Recetas de platillos y sub-recetas (product_recipe)
--      * 48 Sub-recetas asignadas a platillos (product_sub_receta)
--   7. Sincronización de todas las secuencias (setval)
-- ==============================================================================

-- 0. PLANTILLAS DE CAMPAÑAS (campaign_template)
...
BEGIN
    -- 1. App User
    INSERT INTO app_user (full_name, fecha_nacimiento, telefono, email, password_hash, is_active, created_at, updated_at)
    VALUES ('Demo Admin Lealtix', '1990-05-15', '+52 55 1234 5678', 'admin@demo.com', 'YWRtaW4xMjM=', TRUE, NOW(), NOW())
    ON CONFLICT (email) DO UPDATE SET password_hash = 'YWRtaW4xMjM=', is_active = TRUE;

    INSERT INTO app_user (full_name, fecha_nacimiento, telefono, email, password_hash, is_active, created_at, updated_at)
    VALUES ('Administrador Petra', '1990-01-01', '+52 55 1122 3344', 'admin@restaurante-petra.com', 'YWRtaW4xMjM=', TRUE, NOW(), NOW())
    ON CONFLICT (email) DO UPDATE SET password_hash = 'YWRtaW4xMjM=', is_active = TRUE;

    SELECT id INTO v_user_id FROM app_user WHERE email = 'admin@restaurante-petra.com' LIMIT 1;
    IF v_user_id IS NULL THEN
        SELECT id INTO v_user_id FROM app_user WHERE email = 'admin@demo.com' LIMIT 1;
    END IF;

    -- 2. Tenant
    SELECT id INTO v_tenant_id FROM tenant WHERE slug IN ('restaurante-petra', 'demo') ORDER BY id ASC LIMIT 1;
    ...
    IF v_tenant_id IS NULL THEN
        ...
        VALUES (
            ...
            'https://res.cloudinary.com/lealtix-media/image/upload/v1/lealtix/logo_petra.png',
            'Cocina de autor y tradición',
            TRUE, NOW(), TRUE, NOW(), NOW(), v_user_id
        )
        RETURNING id INTO v_tenant_id;
    ELSE
        UPDATE tenant 
        SET kitchen_module_enabled = TRUE, 
            is_active = TRUE,
            nombre_negocio = COALESCE(nombre_negocio, 'Restaurante Petra')
        WHERE id = v_tenant_id;
    END IF;

    -- 3. Tenant Users (Para login en /api/tenant/auth/login con contraseñas Base64)
    INSERT INTO tenant_user (tenant_id, nombre, email, password_hash, rol, activo, sueldo_mensual, created_at, updated_at)
    VALUES
        (v_tenant_id, 'Admin Petra',     'admin@restaurante-petra.com',     'YWRtaW4xMjM=',     'ADMIN',     TRUE, 15000.00, NOW(), NOW()),
        (v_tenant_id, 'Admin Demo',      'admin@demo.com',                  'YWRtaW4xMjM=',     'ADMIN',     TRUE, 15000.00, NOW(), NOW()),
        (v_tenant_id, 'Carlos Mesero',   'mesero@restaurante-petra.com',    'bWVzZXJvMTIz',     'MESERO',    TRUE,  8000.00, NOW(), NOW()),
        (v_tenant_id, 'Carlos Mesero',   'mesero@demo.com',                 'bWVzZXJvMTIz',     'MESERO',    TRUE,  8000.00, NOW(), NOW()),
        (v_tenant_id, 'Ana Cocina',      'cocina@restaurante-petra.com',    'Y29jaW5hMTIz',     'COCINA',    TRUE,  9000.00, NOW(), NOW()),
        (v_tenant_id, 'Ana Cocina',      'cocina@demo.com',                 'Y29jaW5hMTIz',     'COCINA',    TRUE,  9000.00, NOW(), NOW()),
        (v_tenant_id, 'Luis Caja',       'caja@restaurante-petra.com',      'Y2FqYTEyMw==',     'CAJA',      TRUE,  7500.00, NOW(), NOW()),
        (v_tenant_id, 'Luis Caja',       'caja@demo.com',                   'Y2FqYTEyMw==',     'CAJA',      TRUE,  7500.00, NOW(), NOW()),
        (v_tenant_id, 'Sofia Marketing', 'marketing@restaurante-petra.com', 'bWFya2V0aW5nMTIz', 'MARKETING', TRUE, 10000.00, NOW(), NOW()),
        (v_tenant_id, 'Sofia Marketing', 'marketing@demo.com',              'bWFya2V0aW5nMTIz', 'MARKETING', TRUE, 10000.00, NOW(), NOW()),
        (v_tenant_id, 'Maria Hostess',   'hostess@restaurante-petra.com',   'aG9zdGVzczEyMw==', 'HOSTESS',   TRUE,  7000.00, NOW(), NOW()),
        (v_tenant_id, 'Maria Hostess',   'hostess@demo.com',                'aG9zdGVzczEyMw==', 'HOSTESS',   TRUE,  7000.00, NOW(), NOW())
    ...
    INSERT INTO tenant_config (
        ...
        instagram, facebook, kitchen_module_enabled, updated_at
    )
    SELECT
        v_tenant_id,
        'Restaurante Petra: Cocina de autor, tradición e innovación gastronómica.',
        'Ser el restaurante más querido de la ciudad.',
        'contacto@restaurante-petra.com',
        '@restaurantepetra',
        'Restaurante Petra',
        TRUE,
        NOW()
    ...
    -- 5. Mesas
    INSERT INTO mesa (tenant_id, nombre, numero, capacidad, estado, forma, rotacion, posicion_x, posicion_y, created_at, updated_at)
    VALUES
        (v_tenant_id, 'Mesa 1', 1, 4, 'LIBRE', 'cuadrada',    0, 120.0, 100.0, NOW(), NOW()),
        (v_tenant_id, 'Mesa 2', 2, 2, 'LIBRE', 'redonda',     0, 260.0, 100.0, NOW(), NOW()),
        (v_tenant_id, 'Mesa 3', 3, 6, 'LIBRE', 'rectangular', 0, 420.0, 100.0, NOW(), NOW()),
        (v_tenant_id, 'Mesa 4', 4, 4, 'LIBRE', 'cuadrada',    0, 120.0, 260.0, NOW(), NOW()),
        (v_tenant_id, 'Barra',  5, 8, 'LIBRE', 'rectangular', 0, 320.0, 260.0, NOW(), NOW())
    ...
    PERFORM setval('tenant_user_id_seq', COALESCE((SELECT MAX(id) FROM tenant_user), 1));
    PERFORM setval('tenant_id_seq', COALESCE((SELECT MAX(id) FROM tenant), 1));
    PERFORM setval('app_user_id_seq', COALESCE((SELECT MAX(id) FROM app_user), 1));
    PERFORM setval('mesa_id_seq', COALESCE((SELECT MAX(id) FROM mesa), 1));
